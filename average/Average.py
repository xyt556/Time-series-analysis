...
for i in range(0,8,1):
    ...
    ax.plot(angles, data1[i], 'bo-', color=colour[i], linewidth=0.5, label=df['Year'][i])

plt.tight_layout()  #自动调整子图参数,使之填充整个图像区域。
plt.legend(loc="lower right", title="Average", fancybox=False, ncol=1, shadow=True, bbox_to_anchor=(1.25, -0.05))
